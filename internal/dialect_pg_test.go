...
func TestPostgresDialect_FieldDef(t *testing.T) {
	...
	t.Run("identity by default column", func(t *testing.T) {
		// 复现用户场景：新增的自增主键 id 必须输出 GENERATED ... AS IDENTITY，而非裸
		// "bigint NOT NULL"（后者在非空表上 ADD COLUMN 会失败、且不会自增回填既有行）。
		f := &FieldInfo{ColumnName: "id", ColumnType: "bigint", IsNullAble: "NO", IdentityGeneration: "BY DEFAULT"}
		xt.Equal(t, `"id" bigint GENERATED BY DEFAULT AS IDENTITY`, d.FieldDef(f))
	})

	t.Run("identity always column", func(t *testing.T) {
		f := &FieldInfo{ColumnName: "id", ColumnType: "bigint", IsNullAble: "NO", IdentityGeneration: "ALWAYS"}
		xt.Equal(t, `"id" bigint GENERATED ALWAYS AS IDENTITY`, d.FieldDef(f))
	})

	t.Run("identity clause suppresses NOT NULL and DEFAULT", func(t *testing.T) {
		// IDENTITY 隐含 NOT NULL 且不接受独立 DEFAULT，即便这些字段被填充也不应拼出。
		f := &FieldInfo{ColumnName: "id", ColumnType: "integer", IsNullAble: "NO",
			IdentityGeneration: "BY DEFAULT", ColumnDefault: strPtr("0")}
		xt.Equal(t, `"id" integer GENERATED BY DEFAULT AS IDENTITY`, d.FieldDef(f))
	})

	t.Run("uuid column with generator default", func(t *testing.T) {
		// UUID 主键走普通 DEFAULT 路径即可：gen_random_uuid() 会在 ADD COLUMN 时回填既有行，
		// 无需 IDENTITY；确认 UUID 类型与函数默认值都被原样保留。
		f := &FieldInfo{ColumnName: "id", ColumnType: "uuid", IsNullAble: "NO",
			ColumnDefault: strPtr("gen_random_uuid()")}
		xt.Equal(t, `"id" uuid NOT NULL DEFAULT gen_random_uuid()`, d.FieldDef(f))
	})
}

func TestPgIdentityClause(t *testing.T) {
	tests := []struct {
		input string
		want  string
	}{
	...
	for _, tt := range tests {
		t.Run(tt.input, func(t *testing.T) {
			xt.Equal(t, tt.want, pgIdentityClause(tt.input))
		})
	}
}
...
func TestPostgresDialect_GenIndex(t *testing.T) {
	...
	t.Run("drop primary key", func(t *testing.T) {
		...
		sqls := d.GenDropIndex("test", idx)
		xt.Equal(t, []string{`DROP CONSTRAINT IF EXISTS "pk_test"`}, sqls)
	})
...
func TestPostgresDialect_GenForeignKey(t *testing.T) {
	...
	t.Run("drop foreign key", func(t *testing.T) {
		...
		sqls := d.GenDropForeignKey("orders", idx)
		xt.Equal(t, []string{`DROP CONSTRAINT IF EXISTS "fk_user"`}, sqls)
	})
...
func TestPostgresDialect_Idempotent_DropSide(t *testing.T) {
	...
	t.Run("drop index if exists", func(t *testing.T) {
		...
		xt.Equal(t, []string{`DROP INDEX IF EXISTS "idx_a";`}, d.GenDropIndex("t", idx))
	})
	t.Run("drop constraint if exists", func(t *testing.T) {
		idx := &DbIndex{IndexType: indexTypePrimary, Name: "pk_test"}
		xt.Equal(t, []string{`DROP CONSTRAINT IF EXISTS "pk_test"`}, d.GenDropIndex("t", idx))
	})
	t.Run("drop foreign key if exists", func(t *testing.T) {
		idx := &DbIndex{IndexType: indexTypeForeignKey, Name: "fk_user"}
		xt.Equal(t, []string{`DROP CONSTRAINT IF EXISTS "fk_user"`}, d.GenDropForeignKey("orders", idx))
	})
}
